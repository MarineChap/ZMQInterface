...
ZmqInterface::ZmqInterface()
    : GenericProcessor("Zmq Interface")
{
    context = zmq_ctx_new();
    socket = 0;
    flag = 0;
                           ...
                           int64 timestamp, int sampleRate)
{
    ...
    zmq_msg_t message;
    zmq_msg_init_size(&message, sizeof(float)*nRealSamples*nChannels);
    memcpy(zmq_msg_data(&message), static_cast<void*>(real_data->data()), sizeof(float)*nRealSamples*nChannels);
